...
-   **One-time access:** A one-time retrieval of an end user’s loan data, which results in a standardized view of their finances at one point in time.
...
This article explains how to retrieve loan data using continuous access.
...
-   `authorization_code={USER_AUTHORIZATION_CODE}` is used to identify a user in Tink. This value was generated at the end of section 2.
...
```
https://link.tink.com/1.0/products/connect-accounts?products=LOANS&client_id=&state=&redirect_uri=https://console.tink.com/callback&authorization_code=&market=GB&locale=en_US
```

When the end user opens the Tink URL, they're prompted to authenticate with their bank and then choose a bank account. To test a Tink URL, find demo-user credentials in **Console** > **[Demo Bank](https://console.tink.com/demobank)**. Select the username and password for a Demo Bank user that suits your use case.
...
-d 'user_id=' \
-d 'external_user_id=' \
-d 'scope=loan-accounts:readonly,provider-consents:read'
```
...
## 5\. Fetch loan data[](#fetch-loan-data)

### 5.1 Fetch loan accounts[](#fetch-loan-accounts)

To fetch a list of loan accounts for a specific user, you must have a `user access token` with the `loan-accounts:readonly` scope. Use the `user access token` to make a request to the List loan accounts endpoint:
  ...
  "loanAccounts": [
    {
      ...
      "collateral": {
        "collateral": "123 Main Street, New York"
      },
      "dates": {
