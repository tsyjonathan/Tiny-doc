...
source: "https://docs.tink.com/resources/loans/loans-managing-consents"
exportedAt: "2026-01-13T12:45:35.149Z"
---
...
To list provider consents, you must generate a `user access token` with the scope `provider-consents:read`. For instructions on how to generate an access token, see [Get a user access token](https://docs.tink.com/resources/transactions/continuous-connect-to-a-bank-account#get-a-user-access-token).
...
To generate the `authorization_code`, follow the steps in [Generate the code](https://docs.tink.com/resources/transactions/continuous-connect-to-a-bank-account#generate-the-code).

Generate your authorization\_code

```
https://link.tink.com/1.0/transactions/update-consent?client_id=&redirect_uri=&credentials_id=&authorization_code=&market=
```

For more information on which parameters can be provided to the update-consent method in Tink Link, see [Update consent](https://docs.tink.com/resources/transactions/managing-consents#update-a-consent).
...
> **Important note:** Multiple consents that give access to the same account can exist in parallel. When you delete all consents that are related to an account, **the account information and its corresponding loans data is permanently deleted.**

## Delete a user[](#delete-a-user)

When a user is deleted, all consents, related account information, and loans data is also permanently deleted.

To delete a user, you must generate a `user access token` with the scope `user:delete`. See step 4 of this [guide](https://docs.tink.com/resources/transactions/continuous-connect-to-a-bank-account#retrieve-user-data) for instructions on how to generate an access token.
