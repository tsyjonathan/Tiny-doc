...
source: "/Tiny-doc/tink_docs_home/resources/money-manager/money-manager-api/recommend-budgets/"
...
To list recommended budgets, you need to call the [List recommended budgets](/Tiny-doc/tink_docs_api/api/#finance-management/budget/list-recommended-budgets) endpoint using a `user access token` with the `budgets:read` scope. The request takes no parameters.

The response will contain a list of budgets that can be proposed to the user, and later on (if desired) can be created using the [Create recurring budget](/Tiny-doc/tink_docs_api/api/#finance-management/budget/create-recurring-budget) endpoint.
