...
A picture is worth a thousand words, and the same goes for visualising your transfers. In the same moment that transactions and accounts are pushed into the Tink platform, all transactions get categorised. Pre-calculated spending and income statistics are also available for you to fetch and visualise for your application.

In this guide, we will show you how to visualise your monthly transfers across multiple categories.
...
1.  incoming-transfers-by-category
2.  incoming-transfers-by-category/by-count
3.  outgoing-transfers-by-category
4.  outgoing-transfers-by-category/by-count

As part of the request, you will need to include a `user access token` with the `statistics:read` scope.

### incoming-transfers-by-category[](#incoming-transfers-by-category)
...
3.  Set the type to `incoming-transfers-by-category`
...
curl 'https://api.tink.com/api/v1/statistics/query' \
  ...
  --data-raw '{
    "periods": ["2021-04"],
    "resolution": "MONTHLY",
    "types": ["incoming-transfers-by-category"]
  }'
```
...
[
  {
    ...
    "period": "2021-04",
    "resolution": "MONTHLY",
    "type": "incoming-transfers-by-category",
...
In the response, you will receive a category ID for the child categories in the `description` field. The total amount of income in that category during the selected month is represented by the `value` field.

### incoming-transfers-by-category/by-count[](#incoming-transfers-by-category-by-count)

In this example, we want to have the following conditions in our query:

1.  Select the previous month so you can see spending across as many categories in that period
2.  Set the resolution to monthly
3.  Set the type to `incoming-transfers-by-category/by-count`
...
curl 'https://api.tink.com/api/v1/statistics/query' \
  ...
  --data-raw '{
    "periods": ["2021-04"],
    "resolution": "MONTHLY",
    "types": ["incoming-transfers-by-category/by-count"]
  }'
```
...
[
  {
    ...
    "period": "2021-04",
    "resolution": "MONTHLY",
    "type": "incoming-transfers-by-category/by-count",
    "userId": "d9f134ee2eb44846a4e02990ecc8d32e",
    "value": 3.0
  },
  {
...
In the response, you will receive a category ID for the child categories in the `description` field. The amount of incoming transfers in that category during the selected month is represented by the `value` field.

### outgoing-transfers-by-category[](#outgoing-transfers-by-category)

In this example, we want to have the following conditions in our query:

1.  Select the previous month so you can see spending across as many categories in that period
2.  Set the resolution to monthly
3.  Set the type to `outgoing-transfers-by-category`
...
curl 'https://api.tink.com/api/v1/statistics/query' \
  ...
  --data-raw '{
    "periods": ["2021-04"],
    "resolution": "MONTHLY",
    "types": ["outgoing-transfers-by-category"]
  }'
```
...
[
  {
    ...
    "period": "2021-04",
    "resolution": "MONTHLY",
    "type": "outgoing-transfers-by-category",
    "userId": "d9f134ee2eb44846a4e02990ecc8d32e",
    "value": 1298.5
  },
  {
...
### outgoing-transfers-by-category/by-count[](#outgoing-transfers-by-category-by-count)

In this example, we want to have the following conditions in our query:

1.  Select the previous month so you can see spending across as many categories in that period
2.  Set the resolution to monthly
3.  Set the type to `outgoing-transfers-by-category/by-count`
...
curl 'https://api.tink.com/api/v1/statistics/query' \
  ...
  --data-raw '{
    "periods": ["2021-04"],
    "resolution": "MONTHLY",
    "types": ["outgoing-transfers-by-category/by-count"]
  }'
```
...
[
  {
    ...
    "period": "2021-04",
    "resolution": "MONTHLY",
    "type": "outgoing-transfers-by-category/by-count",
    "userId": "d9f134ee2eb44846a4e02990ecc8d32e",
    "value": 14.0
  },
  {
...
In the response, you will receive a category ID for the child categories in the `description` field. The amount of transfers in that category during the selected month is represented by the `value` field.
