---
title: "Business Transactions - Tink Docs"
source: "/Tiny-doc/tink_docs_home/resources/business-transactions/"
exportedAt: "2026-01-13T12:40:17.012Z"
---
[

### Introduction to Business Transactions

Business Transactions provides access to enriched and real-time financial data from business accounts.

![](https://images.ctfassets.net/tmqu5vj33f7w/5ZHBtTg3q37kR1bn4axXZv/23ddaaee0f5d07f72621afe9c0c3d1fd/business-transactions-introduction-teaser3c.jpg)





](/Tiny-doc/tink_docs_home/resources/business-transactions/introduction-to-business-transactions/)

## One-time access

[

![](https://images.ctfassets.net/tmqu5vj33f7w/71fzXQ5qptrPmJhjpr5n4v/1c1848e6b1bccaaece8e421d90d1bd1a/transactions-one-time-access-teaser.jpg)

### One-time access to a business account

This guide will show you how to access business accounts





](/Tiny-doc/tink_docs_home/resources/business-transactions/one-time-access-to-a-business-account/)

## Continuous access

[

![](https://images.ctfassets.net/tmqu5vj33f7w/5ign2dFwFZzxI596mJIxbo/2e009f33f4ff3a46da9d9ae5451cae9f/transactions-continuous-access-teaser.jpg)

### Continuous access to a business account

Continuous access to a business account





](/Tiny-doc/tink_docs_home/resources/business-transactions/continuous-access-to-a-business-account/)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/71m3duCYcnfOIWYX5v6DYM/ab1925779f14947130f01f2ff87bce22/transactions-refreshing-data-teaser.jpg)

### Refresh data

Refresh data when using continuous access





](/Tiny-doc/tink_docs_home/resources/business-transactions/business-refresh-data/)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/7bLAeopeNUFmVF0rED9Zql/9bb440a4d453b1b6c65ed26fa50edcfe/transactions-manage-consent-teaser.jpg)

### Manage consents

Learn how to manage existing consents for access to a business bank account.





](/Tiny-doc/tink_docs_home/resources/business-transactions/business-managing-consents/)

## Fetch data

[

![](https://images.ctfassets.net/tmqu5vj33f7w/2gHOiQgvlfT3sUZgPvK9Eg/7625d85d5df843bf4e89114af820e816/transactions-list-accounts-and-balances.jpg)

### List accounts and balances

To retrieve a list of accounts for a specific user, you need a user access token with the balances:read and accounts:read scope.





](/Tiny-doc/tink_docs_home/resources/business-transactions/business-list-accounts-and-balances/)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/3h18ClE0cJIlixBljsFFkG/9847b4b05853ce959feb6c9c7c49e4cf/business-transactions-list-transactions-teaser3c.jpg)

### List transactions

To retrieve a list of transactions, you need a client access token with the transactions:read scope.





](/Tiny-doc/tink_docs_home/resources/business-transactions/list-business-transactions/)

## Integration

[

![](https://images.ctfassets.net/tmqu5vj33f7w/5HgWGTGQ8LPB1rm14zG5Zs/3bd5d46c5174b9735f17fd39f618f747/setup-and-integrate.jpg)

### Set up and integrate

Learn how to best integrate our SDKs with your app.





](/Tiny-doc/tink_docs_home/resources/business-transactions/setup-and-integrate-business-transactions/)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/7ldtgaQRvdC1JTb9pp28in/a2eaf34c739b0a28f9a607fbb1ff7a94/optimise-your-integration.jpg)

### Optimize your integration

Make sure that your integration is seamless and leads to the highest conversion rates.





](/Tiny-doc/tink_docs_home/resources/business-transactions/optimize-your-business-transactions-integration/)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/KXrtffka9BXEsbJKYrPZK/460bea9658bd1e680c1fda572047e76c/Error_codes_-_Small_card.png)

### Business Transactions errors

Business Transactions error codes and formats, described and explained.





](/Tiny-doc/tink_docs_home/resources/business-transactions/handle-business-transactions-error-codes/)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/Z4iG74v3Cm9X44QnDGs3M/acb3bdea4c48421cff39a45a92c89910/sessions.png)

### Sessions

Launch the SDK with sensitive user information





](/Tiny-doc/tink_docs_home/resources/business-transactions/business-transactions-sdk-sessions/)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/4LxRNYvsyTzhqSH4fDxk3O/be6d28f0b4146e16aee014ba9453f7c2/account-check-SDK-reference.jpg)

### SDK reference

Parameters, responses, versions and market availability for the Business Transactions SDK.





](/Tiny-doc/tink_docs_home/resources/business-transactions/business-transactions-sdk-reference/)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/7jzrdhHTpJPFX62AofaRoc/fdba6c605b022b7ae76960ef556a65bd/Event-notifications-with-webhooks.jpg)

### Webhooks

Set up webhooks to listen for Tink event notifications.





](/Tiny-doc/tink_docs_home/resources/business-transactions/webhooks-for-business-transactions/)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/4nbQXZULzkfjQv6y0ofrUc/abaa6eadf7993e7354cf95afe89eb7fe/transactions-postman.jpg)

### Postman collection

Use our Postman collection for Business Transactions





](/Tiny-doc/tink_docs_home/resources/business-transactions/postman-collection-for-business-transactions/)
