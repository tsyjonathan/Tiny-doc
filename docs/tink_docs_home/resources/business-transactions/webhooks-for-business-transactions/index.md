...
### Available events for Business Transactions[](#available-events-for-business-transactions)

| Event | Description |
| --- | --- |
| `account-transactions:modified` | This event is triggered when an account has new or updated transactions, regardless of their booking status. |
| `account-booked-transactions:modified` | This event is triggered when an account has new or updated transactions that have status `BOOKED`. |
...
| `refresh:finished` | This event is triggered when a refresh operation has finished for a `credential` (for Account Aggregation) or `consent` (for Business Transactions). This can happen for on-demand refreshes and background refreshes. |
...
To validate the signatur, you must recreate it on your end and then compare it. To create a signature, three elements are used:
