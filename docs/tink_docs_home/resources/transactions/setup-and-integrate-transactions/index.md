...
You will first need to construct a URL to launch the SDK flow. In Console, you can use the Tink Link visual editor to easily create and configure the end-user flow. For a list of all possible request and response parameters, see the [Transactions SDK reference](/Tiny-doc/tink_docs_home/resources/transactions/transactions-sdk-reference/). The resulting URL is used to launch the SDK flow, allowing the user to authenticate with their bank and select an account from which Transactions data will be fetched.
