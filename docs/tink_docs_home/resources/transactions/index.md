---
title: "Transactions - Tink Docs"
source: "https://docs.tink.com/resources/transactions"
exportedAt: "2026-01-13T12:40:14.779Z"
---
[

### Introduction to Transactions

Fetch standardized transaction data once or continuously.

![](https://images.ctfassets.net/tmqu5vj33f7w/1iQxiUYxbb5ZMdp9gzvAhb/c84ea33ff5b7c5bd23414fab721de61a/transactions-introduction-teaser2c.jpg)





](https://docs.tink.com/resources/transactions/introduction-to-transactions)

## One-time access

[

![](https://images.ctfassets.net/tmqu5vj33f7w/71fzXQ5qptrPmJhjpr5n4v/1c1848e6b1bccaaece8e421d90d1bd1a/transactions-one-time-access-teaser.jpg)

### One-time access to a bank account

Learn how to make a one-time connection to a bank account





](https://docs.tink.com/resources/transactions/connect-to-a-bank-account)

## Continuous access

[

![](https://images.ctfassets.net/tmqu5vj33f7w/5ign2dFwFZzxI596mJIxbo/2e009f33f4ff3a46da9d9ae5451cae9f/transactions-continuous-access-teaser.jpg)

### Continuous access to a bank account

Learn how to get continuous access to a user’s bank account





](https://docs.tink.com/resources/transactions/continuous-connect-to-a-bank-account)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/71m3duCYcnfOIWYX5v6DYM/ab1925779f14947130f01f2ff87bce22/transactions-refreshing-data-teaser.jpg)

### Refresh data

Refresh data when using continuous access





](https://docs.tink.com/resources/transactions/refresh-data)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/7bLAeopeNUFmVF0rED9Zql/9bb440a4d453b1b6c65ed26fa50edcfe/transactions-manage-consent-teaser.jpg)

### Manage consents

Learn how to manage, update and delete consents for your users





](https://docs.tink.com/resources/transactions/managing-consents)

## Fetch data

[

![](https://images.ctfassets.net/tmqu5vj33f7w/2gHOiQgvlfT3sUZgPvK9Eg/7625d85d5df843bf4e89114af820e816/transactions-list-accounts-and-balances.jpg)

### List accounts and balances

Fetch a list of accounts for a specific user.





](https://docs.tink.com/resources/transactions/list-accounts-and-balances)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/2nMEd7OWeL3VqUMIbpl5nP/936d57e1c57ed203bc36879552c78410/transactions-list-transactions.jpg)

### List transactions

Filter and fetch a list of transactions.





](https://docs.tink.com/resources/transactions/list-transactions)

## Integration

[

![](https://images.ctfassets.net/tmqu5vj33f7w/5HgWGTGQ8LPB1rm14zG5Zs/3bd5d46c5174b9735f17fd39f618f747/setup-and-integrate.jpg)

### Set up and integrate

Learn how to best integrate our SDKs with your app.





](https://docs.tink.com/resources/transactions/setup-and-integrate-transactions)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/7ldtgaQRvdC1JTb9pp28in/a2eaf34c739b0a28f9a607fbb1ff7a94/optimise-your-integration.jpg)

### Optimize your integration

Make sure that your integration is seamless and leads to the highest conversion rates.





](https://docs.tink.com/resources/transactions/optimize-your-transactions-integration)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/KXrtffka9BXEsbJKYrPZK/460bea9658bd1e680c1fda572047e76c/Error_codes_-_Small_card.png)

### Transactions errors

Transactions error codes and formats, described and explained.





](https://docs.tink.com/resources/transactions/handle-transactions-error-codes)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/Z4iG74v3Cm9X44QnDGs3M/acb3bdea4c48421cff39a45a92c89910/sessions.png)

### Sessions

Launch the SDK with sensitive user information





](https://docs.tink.com/resources/transactions/transactions-sdk-sessions)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/4LxRNYvsyTzhqSH4fDxk3O/be6d28f0b4146e16aee014ba9453f7c2/account-check-SDK-reference.jpg)

### SDK reference

Parameters, responses, versions and market availability for the Transactions SDK.





](https://docs.tink.com/resources/transactions/transactions-sdk-reference)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/sUsbBwsjacT5bNgvzRXkB/8be82e8a66d88b0cb3a0f965feb245e9/Transactions_Early_Redirect.png)

### Early redirect

Redirect users back to your app right after they've authenticated.





](https://docs.tink.com/resources/transactions/early-redirect)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/7jzrdhHTpJPFX62AofaRoc/fdba6c605b022b7ae76960ef556a65bd/Event-notifications-with-webhooks.jpg)

### Webhooks

Set up webhooks to listen for Tink event notifications.





](https://docs.tink.com/resources/transactions/webhooks-for-transactions)

[

![](https://images.ctfassets.net/tmqu5vj33f7w/4nbQXZULzkfjQv6y0ofrUc/abaa6eadf7993e7354cf95afe89eb7fe/transactions-postman.jpg)

### Postman collections

Use our Postman collections for Transactions





](https://docs.tink.com/resources/transactions/postman-collections-for-transactions)
