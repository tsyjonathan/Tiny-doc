---
title: "Integrate Transactions in iOS apps"
source: "https://docs.tink.com/resources/transactions/integrate-transactions-in-ios-apps"
exportedAt: "2026-01-13T12:46:12.022Z"
---
...
Build a URL to launch the SDK by either using the Tink Link Builder in [Tink Console](https://console.tink.com/) or constructing a URL and providing the [required parameters](https://docs.tink.com/resources/transactions/transactions-sdk-reference) as well as the `app_uri` and `redirect_uri` you defined previously.
...
```
https://link.tink.com/1.0/transactions/connect-accounts?client_id={YOUR_CLIENT_ID}&market=SE&locale=en_US&redirect_uri=example%3A%2F%2Fcallback&app_uri=example%3A%2F%2Fopen&auto_redirect_mobile=true
```
    ...
    override func viewDidLoad() { 
        ...
        NotificationCenter.default.addObserver(forName: .linkCallback, object: nil, queue: .main) { [weak self] notification in 
            ...
            if let code = components?.queryItems?.first(where: { $0.name == "code" })?.value { 
                // Successful response: Use `code` to retrieve a user access token that can be used to access the transactions
...
See the [Transactions SDK reference](https://docs.tink.com/resources/transactions/transactions-sdk-reference) for success and error response formats and parameters.
