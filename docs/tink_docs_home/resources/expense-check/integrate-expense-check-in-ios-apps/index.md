---
title: "Integrate Expense Check in iOS apps"
source: "/Tiny-doc/tink_docs_home/resources/expense-check/integrate-expense-check-in-ios-apps/"
exportedAt: "2026-01-13T12:49:06.369Z"
---
...
Build a URL to launch the SDK by either using the Tink Link Builder in [Tink Console](https://console.tink.com/) or constructing a URL and providing the [required parameters](/Tiny-doc/tink_docs_home/resources/expense-check/expense-check-sdk-reference/) as well as the `app_uri` and `redirect_uri` you defined previously.
...
```
https://link.tink.com/1.0/expense-check/create-report?client_id={YOUR_CLIENT_ID}&market=SE&locale=en_US&redirect_uri=example%3A%2F%2Fcallback&app_uri=example%3A%2F%2Fopen&auto_redirect_mobile=true
```
...
final class LinkViewController: UIViewController { 
    ...
    override func viewDidLoad() { 
        ...
        NotificationCenter.default.addObserver(forName: .linkCallback, object: nil, queue: .main) { [weak self] notification in 
            ...
            if let expenseCheckId = components?.queryItems?.first(where: { $0.name == "expense_check_id" })?.value { 
                // Successful response: Use `expenseCheckId` to retrieve the report using your backend
...
The SDK will return the result of the operation as query parameters appended to the `redirect_uri`. You can access the `expense_check_id` in the `Notification.Name.linkCallback` notification observer as demonstrated in the [preconfigured WKWebView example](#open-the-url-in-a-preconfigured-wkwebview).

See the [Expense Check SDK reference](/Tiny-doc/tink_docs_home/resources/expense-check/expense-check-sdk-reference/) for success and error response formats and parameters.
