...
source: "/Tiny-doc/tink_docs_home/resources/expense-check/fetch-data-from-several-risk-products-in-one-flow/"
exportedAt: "2026-01-13T12:49:01.041Z"
---
...
| --- | --- | --- |
| async | ✅ | Set this to `true` to enable early redirect. Set this to `false` to disable early redirect. |
| client\_id | ✅ | The `client_id` for your app. |
| redirect\_uri | ✅ | The URL to which the user is redirected. This must be configured in the app settings in Console before first use. |
| market | ✅ | The market to fetch the reports for e.g. `SE`. |
| report\_types | ✅ | Report type. To use both report types, separate the values by using a comma. Options: `INCOME_CHECK_REPORT, RISK_INSIGHTS_REPORT, EXPENSE_CHECK_REPORT` |
| external\_reference | ❌ | Optional reference given by the customer to add information, such as UID, to the generated report. |
