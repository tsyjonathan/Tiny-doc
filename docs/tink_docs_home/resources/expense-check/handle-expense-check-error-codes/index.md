...
source: "https://docs.tink.com/resources/expense-check/handle-expense-check-error-codes"
...
Something is wrong with the Tink Link URL, usually due to missing or incorrect parameters. For the full list of parameters and values, see the [Expense Check SDK reference](https://docs.tink.com/resources/expense-check/expense-check-sdk-reference).
...
| `INVALID_PARAMETER_CREDENTIALS_ID` | The `credentials_id` value is missing. Used only with [permanent users](https://docs.tink.com/resources/expense-check/resources/aggregation/permanent-users). |
...
| `INVALID_STATE_CREDENTIALS_IS_AWAITING_THIRD_PARTY` | Attempted authentication or refresh of a credential that is in a pending state. Used only with [permanent users](https://docs.tink.com/resources/expense-check/resources/aggregation/permanent-users). |
...
| `INVALID_STATE_NON_OPEN_BANKING_PROVIDER` | Attempted authentication of a credential for a non-open-banking provider. Used only with [permanent users](https://docs.tink.com/resources/expense-check/resources/aggregation/permanent-users). |
