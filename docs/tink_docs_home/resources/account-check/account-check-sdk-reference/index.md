---
title: "Account Check SDK reference - Tink Docs"
source: "/Tiny-doc/tink_docs_home/resources/account-check/account-check-sdk-reference/"
exportedAt: "2026-01-13T12:43:52.725Z"
---
This reference describes the parameters, success and error responses, and market and language availability for the Account Check SDK.

Account Check base URL
...
| `refreshable_items` | Optional | Account types. To use more than one, separate the values by using a comma. Options: `IDENTITY_DATA, CHECKING_ACCOUNTS, SAVING_ACCOUNTS`. When this parameter is enabled and account types are not specified, the defaults are used, which are: `IDENTITY_DATA`, `CHECKING_ACCOUNTS`, and `SAVING_ACCOUNTS`. |
...
```
https://link.tink.com/1.0/account-check/create-report?client_id=&redirect_uri=http://localhost:3000/callback&market=GB
```
...
| --- | --- | --- |
| `account_verification_report_id` | Always | Identifier that can be used to retrieve the generated account verification report. |
...
| Parameter | Availability | Description |
| --- | --- | --- |
| `error` | Always | A status code that describes the category of the error. For details, see [error statuses](/Tiny-doc/tink_docs_home/resources/account-check/handle-account-check-error-codes/#error-statuses). |
| `error_reason` | Always | The error reason that describes the cause of the error. For details, see the error reasons for the respective [error status](/Tiny-doc/tink_docs_home/resources/account-check/handle-account-check-error-codes/#error-statuses). |
...
These are the markets supported for Account Check. If a `market` isn't provided, Tink defaults to `SE`. See [market capabilities](/Tiny-doc/tink_docs_home/market-capabilities/) for all available providers by market.
