---
title: "Integrate Account Check in iOS apps"
source: "https://docs.tink.com/resources/account-check/integrate-account-check-in-ios-apps"
exportedAt: "2026-01-13T12:43:39.688Z"
---
...
final class LinkViewController: UIViewController { 
    ...
    override func viewDidLoad() { 
        ...
        NotificationCenter.default.addObserver(forName: .linkCallback, object: nil, queue: .main) { [weak self] notification in 
            ...
            if let accountVerificationReportId = components?.queryItems?.first(where: { $0.name == "account_verification_report_id" })?.value { 
                // Successful response: Use `accountVerificationReportId` to retrieve the report using your backend
...
The SDK will return the result of the operation as query parameters appended to the `redirect_uri`. You can access the `account_verification_report_id` in the `Notification.Name.linkCallback` notification observer as demonstrated in the [preconfigured WKWebView example](#open-the-url-in-a-preconfigured-wkwebview).

See the [Account Check SDK reference](https://docs.tink.com/resources/account-check/account-check-sdk-reference) for success and error response formats and parameters.
