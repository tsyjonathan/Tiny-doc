...
```
https://link.tink.com/1.0/business-account-check/create-report?client_id={YOUR_CLIENT_ID}&redirect_uri=https://console.tink.com/callback&market=GB
```
...
1.  Initiate and present an instance of the view controller using your configuration and desired market.
    ...
    ```
    let viewController = Tink.BusinessAccountCheck.createReport(configuration: configuration, market: Market(<#String#>)) { result in
        // Handle result
    }
...
3.  You are responsible for dismissing the `viewController` by calling `dismiss(animated:completion:)` inside the completion handler.
    ...
    ```
        let viewController = Tink.BusinessAccountCheck.createReport(configuration: configuration, market: Market(<#String#>)) { [weak self] result in
            // Handle result
...
For the full reference documentation, see the [iOS SDK Reference](https://tink-ab.github.io/tink-link-ios/documentation/tinklink/tink/businessaccountcheck).
...
2.  Create the request object for the specific product flow:
    ...
    ```
    val request = BusinessAccountCheckCreateReport()
    ```
...
For the full reference documentation, see the [Android SDK Reference](https://tink-ab.github.io/tink-link-android/-tink%20-link%20-android%20-s-d-k/com.tink.link.core.features.businessAccountCheck/-tink-business-account-check/index.html).
