...
You will first need to construct a URL to launch the SDK flow. In Console, you can use the Tink Link visual editor to easily create and configure the end-user flow. For a list of all possible request and response parameters, see the [Business Account Check SDK reference](/Tiny-doc/tink_docs_home/resources/business-account-check/business-account-check-sdk-reference/). The resulting URL is used to launch the SDK flow, allowing the user to authenticate with their bank and select an account that will be used to generate the account verification report.
