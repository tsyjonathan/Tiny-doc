...
```
https://link.tink.com/1.0/income-check/create-report?client_id={YOUR_CLIENT_ID}&market=SE&locale=en_US&redirect_uri=example%3A%2F%2Fcallback&app_uri=example%3A%2F%2Fopen&auto_redirect_mobile=true
```
...
override fun onNewIntent(intent: Intent?) {
    ...
    intent?.data?.let { uri ->
        if (uri.host == "callback") {
            val incomeCheckId = uri.getQueryParameter("income_check_id")
            ... // Use `incomeCheckId` to retrieve the report using your backend
        }
    }
}
```
