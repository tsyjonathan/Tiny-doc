---
title: "Set up and integrate Income Check"
source: "https://docs.tink.com/resources/income-check/setup-and-integrate-income-check"
exportedAt: "2026-01-13T12:48:51.564Z"
---
...
You will first need to construct a URL to launch the SDK flow. In [Console](https://console.tink.com/), you can use the [Tink Link visual editor](https://console.tink.com/income-check/tink-link) to easily create and configure the end-user flow. For a list of all possible request and response parameters, see the [Income Check SDK reference](https://docs.tink.com/resources/income-check/income-check-sdk-reference). The resulting URL is used to launch the SDK flow, allowing the user to authenticate and generate the income report.
...
```
{YOUR_REDIRECT_URI}?income_check_id=ff8ae53bc46e45fe9a37c4fd1353e60d
```

See the [Income Check SDK reference](https://docs.tink.com/resources/income-check/income-check-sdk-reference) for success and error response formats and their parameters.

To successfully integrate the web SDK in a mobile app, see [using the web SDK in mobile apps](https://docs.tink.com/resources/income-check/using-the-income-check-web-sdk-in-mobile-apps).

### API Reference[](#api-reference)

For the full reference documentation, see the [Income Check SDK reference](https://docs.tink.com/resources/income-check/income-check-sdk-reference).
...
1.  Initiate and present an instance of the view controller using your configuration and desired market.
    ...
    ```
    let viewController = Tink.IncomeCheck.createReport(configuration: configuration, market: Market(<#String#>)) { result in
        // Handle result
    }
...
3.  You are responsible for dismissing the `viewController` by calling `dismiss(animated:completion:)` inside the completion handler.
    ...
    ```
        let viewController = Tink.IncomeCheck.createReport(configuration: configuration, market: Market(<#String#>)) { [weak self] result in
            // Handle result
