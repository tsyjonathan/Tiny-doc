...
source: "https://docs.tink.com/resources/payments/use-only-one-sca-with-permanent-users"
...
For more information on how to create permanent users, see [Permanent users](https://docs.tink.com/resources/tink-link-web/tink-link-web-permanent-users).

For more information on how source account preselection works, see [Source account preselection](https://docs.tink.com/resources/payments/payment-initiation-flow-optimisation-source-account-preselection).
...
For users that repeat payment initiation at least twice in 90 days, reuse the PSD2 consent that they provided as part of the [AIS + PIS flow](https://docs.tink.com/resources/payments/payment-initiation-flow-optimisation-overview#quot-ais-pis-quot-flow).

For information on which flow is supported by which bank, see [Payment conditions](https://docs.tink.com/resources/payments/payment-conditions).
...
This scenario is suitable if you bundle Payments with [Account Check](https://docs.tink.com/resources/account-check) or [Transactions](https://docs.tink.com/resources/transactions).
...
Note: further optimize the flow by [preselecting the source account](https://docs.tink.com/resources/payments/payment-initiation-flow-optimisation-source-account-preselection). This reduces the number of screens, which is likely to increase your conversion rate.
...
To create a user, create a [client access token](https://docs.tink.com/resources/getting-started/retrieve-access-token) with the scopeS `user:create` and `provider-consents:read`. For more information, see the [Create a permanent user](https://docs.tink.com/resources/tink-link-web/tink-link-web-permanent-users#generate-a-user-authorization-code) section of the _Permanent users_ article.

Tink must be granted access to perform certain actions on the user that you have created. To do this, you delegate access from your product to your Tink app. For information on how to do this, see the [Generate a user authorization code](https://docs.tink.com/resources/tink-link-web/tink-link-web-permanent-users#generate-a-user-authorization-code) section of the _Permanent users_ article.

To delegate authorization, you must use a [client access token](https://docs.tink.com/resources/getting-started/retrieve-access-token) with the `authorization:grant` and `provider-consents:read` scopes.
...
To create your first payment, follow all steps in [Initiate your first payment](https://docs.tink.com/resources/payments/start-payment).
...
To create a user, you must first create a [client access token](https://docs.tink.com/resources/getting-started/retrieve-access-token) with the scope `user:create`. For more information, see the [Create a permanent user](https://docs.tink.com/resources/tink-link-web/tink-link-web-permanent-users#create-a-permanent-user) section of the _Permanent users_ article.
...
To delegate authorization, you must use a [client access token](https://docs.tink.com/resources/getting-started/retrieve-access-token) with the scope `authorization:grant`.
...
To [add credentials](https://docs.tink.com/api#connectivity/credentials) to either a new or existing user (who may have other credentials already), initiate the Tink Link add credentials flow and specify the authorization\_code as a query parameter on the URL.
...
For a list of all the available parameters, see [Tink Link API reference for Account Aggregation](https://docs.tink.com/resources/tink-link-web/tink-link-web-api-reference-account-aggregation).
