...
```
https://link.tink.com/1.0/pay/vrp-mandate?client_id={YOUR_CLIENT_ID}&consent_id={YOUR_CONSENT_ID}&authorization_code={YOUR_AUTHORIZATION_CODE}&redirect_uri={YOUR_REDIRECT_URI}
```
...
let configuration = Configuration(clientID: <#String#>, redirectURI: <#String#>, market: Market, baseDomain: <#BaseDomain#>)
```

### Launching the SDK[](#launching-the-sdk)

1.  Initiate and present an instance of the view controller using your `configuration`, `consentID` and `authorizationCode`.
    
    ```
    let viewController = Tink.Payments.authorizeRecurringPaymentsMandate(configuration: configuration, consentID: consentID, authorizationCode: authorizationCode) { result in
        // Handle result
    }
...
3.  You are responsible for dismissing the `viewController` by calling `dismiss(animated:completion:)` inside the completion handler.
    ...
    ```
        let viewController = Tink.Payments.authorizeRecurringPaymentsMandate(configuration: configuration, consentID: consentID, authorizationCode: authorizationCode) { [weak self] result in
            // Handle result
...
For the full reference documentation, see the [iOS SDK Reference](https://tink-ab.github.io/tink-link-ios/documentation/tinklink/tink/payments).
...
2.  Create the request object for the specific product flow:
    ...
    ```
    val request = AuthorizeRecurringPaymentsMandate()
    ```
...
3.  Launch the product flow by passing the configuration and request object. You can display the SDK in two different modes by specifying the launchMode parameter (either FullScreen or Modal):
    ...
    ```
    Tink.Payments.authorizeRecurringPaymentsMandate(
        activity,
        configuration,
        request,
        FullScreen(...),
...
For the full reference documentation, see the [Android SDK Reference](https://tink-ab.github.io/tink-link-android/-tink%20-link%20-android%20-s-d-k/com.tink.link.core.features.payments/-tink-payments/index.html).
