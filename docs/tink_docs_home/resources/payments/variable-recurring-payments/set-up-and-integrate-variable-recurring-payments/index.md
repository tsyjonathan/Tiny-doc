...
You will first need to construct a URL to launch the SDK flow. In Console, you can use the Tink Link visual editor to easily create and configure the end-user flow. For a list of all possible request and response parameters, see the [Variable Recurring Payments SDK reference](/Tiny-doc/tink_docs_home/resources/payments/variable-recurring-payments/variable-recurring-payments-sdk-reference/). The resulting URL is used to launch the SDK flow, allowing the user to authorize a sweeping VRP mandate.
