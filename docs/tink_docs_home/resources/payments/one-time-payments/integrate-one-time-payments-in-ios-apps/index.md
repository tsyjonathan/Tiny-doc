---
title: "Integrate One-Time Payments in iOS apps"
source: "https://docs.tink.com/resources/payments/one-time-payments/integrate-one-time-payments-in-ios-apps"
exportedAt: "2026-01-13T12:42:07.482Z"
---
...
```
https://link.tink.com/1.0/pay/direct?client_id={YOUR_CLIENT_ID}&market=SE&locale=en_US&payment_request_id={YOUR_PAYMENT_REQUEST_ID}&redirect_uri=https%3A%2F%myapp.com%2Fcallback&app_uri=https%3A%2F%myapp.com%open&auto_redirect_mobile=true
```
...
final class LinkViewController: UIViewController { 
    ...
    override func viewDidLoad() { 
        ...
        NotificationCenter.default.addObserver(forName: .linkCallback, object: nil, queue: .main) { [weak self] notification in 
            ...
            if let paymentRequestId = components?.queryItems?.first(where: { $0.name == "payment_request_id" })?.value { 
                // Successful response: Use `paymentRequestId` to retrieve information about your payment or transfer
...
The SDK will return the result of the operation as query parameters appended to the `redirect_uri`. You can access the `payment_request_id` in the `Notification.Name.linkCallback` notification observer as demonstrated in the [preconfigured WKWebView example](#open-the-url-in-a-preconfigured-wkwebview).

See the [One-time payments SDK reference](https://docs.tink.com/resources/payments/one-time-payments/one-time-payments-sdk-reference) for success and error response formats and parameters.
