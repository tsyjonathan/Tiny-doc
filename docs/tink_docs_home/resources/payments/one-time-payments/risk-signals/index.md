...
source: "/Tiny-doc/tink_docs_home/resources/payments/one-time-payments/risk-signals/"
...
To ensure the most reliable transaction history checks, we recommend that you add a unique reference (`remittanceInformation`) when creating the [payment request](/Tiny-doc/tink_docs_api/api/#payment/payment-request/create-payment-request).
...
When this feature is enabled, you can pass in the account holder’s name and/or their account details as part of the `sender` object in your [payment request](/Tiny-doc/tink_docs_api/api-payment/#payment/payment-request). Tink will perform either an exact match or partial match depending on how name check has been configured and will always check for exact matches for account numbers.
...
To learn more about this check, see [User Match](/Tiny-doc/tink_docs_home/resources/account-check/user-match/). To enable this check, reach out to your Tink representative.
...
When polling for payment status when Risk Signals is enabled, you’ll get a new status code `422` for payments that fail due to being aborted by Risk Signals. This status is returned by the  [Get Transfer endpoint](/Tiny-doc/tink_docs_api/api/#payment/payment-request/get-transfers-for-payment-request) and our webhook event [payment:updated](/Tiny-doc/tink_docs_home/resources/payments/one-time-payments/one-time-payments-notifications-and-webhooks/#event-payment-updated) in the `statusMessage` field.

Tink Customers will also receive a new error code with the `FAILED` status of the payment. Error code for Risk signals is `422` with message `Payment blocked`. This is returned with the status when customer does their reconciliation with the [Get Transfer endpoint](/Tiny-doc/tink_docs_api/api/#payment/payment-request/get-transfers-for-payment-request) or our webhook event [payment:updated](/Tiny-doc/tink_docs_home/resources/payments/one-time-payments/one-time-payments-notifications-and-webhooks/#event-payment-updated) in the `statusMessage` field.
...
-   Initiate a Demobank payment according to our regular [guide](/Tiny-doc/tink_docs_home/resources/payments/one-time-payments/test-different-one-time-payment-scenarios/). When creating the payment request, set the `amount` to **above** 999 EUR if testing blocked payments or **under** 999 EUR if testing successful payments.
