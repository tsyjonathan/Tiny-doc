...
source: "https://docs.tink.com/resources/investments/investments-refresh-data"
exportedAt: "2026-01-13T12:45:02.095Z"
---
...
The [Provider Consent](https://docs.tink.com/api#connectivity/provider-consent) endpoint gives details about the state of end-user consents with financial institutions.
...
We limit excessive amounts of API requests per app. If your request is rate limited, you will receive a 204 response here, but the status will not change in step 4. For more information on rate limiting, see [Rate limits](https://docs.tink.com/api#introduction/rate-limits).
...
## 4\. Poll for changes[](#poll-for-changes)

Monitor the refresh progress by polling the Provider Consent endpoint every 1-2 seconds. To do this, repeat the API call in section 1.
...
-   **Refresh successful:** The `statusUpdated` timestamp has changed, and the `status` is now `UPDATED`. You can now go ahead and fetch updated [list of investment accounts](https://docs.tink.com/api#data-v2/investment/list-investment-accounts) and [holdings](https://docs.tink.com/api#data-v2/investment/list-holdings).
-   **Refresh in progress:** Statuses such as `UPDATING` and `AUTHENTICATING` indicate that the refresh is in process. Keep polling and monitor for further changes.
...
If the refresh request ended in one of the `statuses` listed above for unsuccesful refreshes, then you will need to [direct the end user to **update their consent**](https://docs.tink.com/resources/investments/investments-managing-consents).
