...
source: "https://docs.tink.com/resources/aggregation/permanent-users"
exportedAt: "2026-01-13T12:55:41.063Z"
---
## Overview[](#overview)

Use a Tink URL with a permanent user by following these steps:

1.  Create a permanent user
2.  Generate a user authorization code
3.  Launch a Tink URL by using the user authorization code

## 1\. Create a permanent user[](#create-a-permanent-user)

To create a user, you must first create a [client access token](https://docs.tink.com/resources/getting-started/retrieve-access-token) with the scope `user:create`.

### Example of creating a client access token[](#example-of-creating-a-client-access-token)

See the [get access token](https://docs.tink.com/api#general/oauth/get-access-token) API reference for details.
...
See the [create user](https://docs.tink.com/api#general/user/create-user) API reference for details.
...
Tink must be granted access to perform actions on the permanent user. This is done by delegating access from your client to the Tink Link client. To delegate access to the Tink Link client, you will need to use the [authorization delegate endpoint](https://docs.tink.com/api#general/oauth/create-delegated-authorization) to retrieve an authorization code and propagate it to Tink.
...
See the [create user](https://docs.tink.com/api#general/oauth/get-access-token) API reference for details.
...
See the [create delegated authorization](https://docs.tink.com/api#general/oauth/create-delegated-authorization) API reference for details.
...
You can use either `user_id` or `external_user_id` to reference the permanent user, for more information see [create a delegated authorization code](https://docs.tink.com/api#general/oauth/create-delegated-authorization).
...
[Adding credentials](https://docs.tink.com/api#connectivity/credentials) for new users or existing users (with possibly other existing credentials), is done by initiating the Tink Link add credentials flow and specifying the `authorization_code` query parameter.
...
For a list of all the available parameters, please see the [Tink Link API reference for Account Aggregation](https://docs.tink.com/resources/tink-link-web/tink-link-web-api-reference-account-aggregation).
...
[Authenticating credentials](https://docs.tink.com/api#connectivity/credentials) allows you to extend the session of an existing credentials without refreshing the data. Authenticating credentials is done by initiating the Tink Link authenticate credentials flow and specifying the `authorization_code` and `credentials_id` query parameters.
...
| payment\_request\_id | Required. The [created payment request identifier](https://docs.tink.com/resources/payments/start-payment#create-a-payment-request). |
